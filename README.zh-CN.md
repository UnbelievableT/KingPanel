...
*总览 —— 资金曲线、20 项核心统计、EA/手工交易占比。*

![分析](docs/analysis.png)
*分析 —— 分周期统计表，含最大回撤与回撤比例，可排序，日/周/月/年。*

![日历](docs/news.png)
*财经日历 —— 星级过滤、货币筛选、护盾开关、K 线标注。*

![图表中心](docs/charts.png)
*图表中心 —— 单笔 MFE×MAE 散点、E-Ratio 与波段利用效率（六个组合页之一）。*

彭博终端风格的 MT5 账户统计看板 EA。纯 `CCanvas` 位图渲染 —— 深色终端底、琥珀色主调、直角边框、1px 分割线、等宽数字字体，紧凑高密度布局，DPI 自适应（4K/缩放屏自动放大），支持拖拽与收拢。

**V1.5 要点**：两轮对抗式审计——**99 个已验证修复**（六视角代码审计 71 项，随后独立验证又抓出修复自身引入的 28 个回归）：下单安全（停止位/保证金预校验；风控守卫在平仓被拒时保持武装持续重试、事件只播报一次，不再静默自我解除）；统计口径（浮动盈亏剔除赠金、反向单拆分为平仓/开仓两段并重置入场基准、MFE/MAE 缓存按窗口修剪）；**原子化单实例所有权**（CAS 抢占 + 降级 + 镜像标识）；任意屏幕文字清晰（XRGB 画布 + DPI 自适应 **InpFontScale** + 值文本 CJK 字体自动切换）。MFE/MAE 样本不再饿死：旧的持久化黑名单在挂载后几秒内就判死了所有仓位（1 秒定时器在历史还没下载完时就用光了三次重试，判决还被缓存四周）。现改为内存内 30 秒退避重试；当 M1 确实拿不到时（终端限制了保留柱数，或券商不提供那么久远的 M1），样本自动降级到 M5/M15/M30/H1，并在页面上标明实际使用的 K 线精度，而不是永远空白。时段热力图页自解释化：坐标系标题（星期 × 平仓小时）、格子颜色/数字/行尾合计/时段色带的图例行、带城市名的开市时段带。
...
**V1.1-V1.2 要点**：中英双语（默认英文，标题栏一键切换）；金色图表主题（卸载自动还原）；**下单面板**（市价/四类挂单、手数/止损止盈/距离步进、一键下单）；图表中心五张大图（资金曲线/日盈亏/月盈亏/回撤水下图/**MFE×MAE 散点**——对称刻度方图 + 绿红象限底色 + E-Ratio/兑现率）；持仓页挂单子页；分析表**周期最大回撤**列，总览含今日/本月回撤；日历**币种筛选**（默认 USD/EUR/JPY，10 币种 chip）+ K 线竖线与**错落式旋转标签** + 星级弹窗提醒；面板宽度可调（默认 560，美分账户友好）；全部面板填充为**预混合不透明色**（修复可透视看到背景 K 线的问题）。
